...
describe('CategorySequelizeRepository Integration Tests', () => {
  ...
  beforeEach(async () => {
    categorySequelizeRepository = new CategorySequelizeRepository(
      CategoryModel,
    );
  });
  ...
  it('should find a category by id', async () => {
    ...
    const categoryFound = await categorySequelizeRepository.findById(
      category.categoryId,
    );
  ...
  it('should find a category returning null when not found', async () => {
    const categoryFound = await categorySequelizeRepository.findById(
      Category.fake().aCategory().build().categoryId,
    );
  ...
  it('should find all categories', async () => {
    ...
    expect(categoriesFound.map((c) => c.toJSON())).toEqual(
  ...
  it('should not find a category by id', async () => {
    ...
    const categoryFound =
      await categorySequelizeRepository.findById(categoryId);
  ...
  it('should search categories with no params', async () => {
    ...
    const categoriesFound = await categorySequelizeRepository.search(
      new CategorySearchParams(),
    );
    expect(categoriesFound.items.length).toBe(15);
    expect(categoriesFound.items.map((c) => c.toJSON())).toEqual(
      categoriesToSearch.map((c) => c.toJSON()),
    );
  });
  ...
  it('should search categories with pagination', async () => {
    ...
    const categoriesFound = await categorySequelizeRepository.search(
      new CategorySearchParams({ page: 1, per_page: 5 }),
    );
  ...
  it('should search categories with filter', async () => {
    ...
    const categoriesFound = await categorySequelizeRepository.search(
      new CategorySearchParams({ filter: categories[0].name }),
    );
  ...
  it('should search categories with sort', async () => {
    const categories = Category.fake().theCategories(16).build();
    // get by categories order by createdAt desc and slice the last one
    const categoriesToSearch = categories.sort((a, b) =>
    ...
    const categoriesFound = await categorySequelizeRepository.search(
      new CategorySearchParams({ sort: 'name', sort_dir: 'asc', per_page: 16 }),
    );
  ...
  it('should return not found when try to update a category that does not exist', async () => {
    const category = Category.fake().aCategory().build();
    await expect(categorySequelizeRepository.update(category)).rejects.toThrow(
  ...
  it('should return not found when try to delete a category that does not exist', async () => {
    const category = Category.fake().aCategory().build();
    await expect(
      categorySequelizeRepository.delete(category.categoryId),
    ).rejects.toThrow(new NotFoundError(category.categoryId.id, Category));
  });
});
